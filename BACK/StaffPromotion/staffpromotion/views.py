...
def teaching_evaluations(request):
    ...
    # ============================================================
    # GET STUDENT TEACHING EVALUATIONS
    # ============================================================
    ...
    if request.method == "GET":
        ...
        # --------------------------------------------------------
            ...
            evaluations = StudentTeachingEvaluation.objects.all()
        ...
        if application_id:
            ...
            # IMPORTANT
            #
            # Student evaluations are linked to the instructor.
            # Therefore only evaluations belonging to the employee
            # who owns this promotion application are returned.
            # ----------------------------------------------------

            evaluations = evaluations.filter(
                instructor=application.employee
            )
...
def process_promotion_decision(request, pk):
    ...
    IMPORTANT:
    Reviewer completion does NOT automatically mean that the
    application is ready for Committee unless ALL required
    academic reviews AND student evaluations are completed.
    """
    ...
    if role == "HOD":
        ...
        if application.status not in [
            ...
            return Response(
                {
                    "error":
                    "This application is not currently available for HOD review.",
        ...
        if decision not in [
            ...
            return Response(
                {
                    "error":
                    "Invalid HOD decision. Use RECOMMENDED or REJECTED."
                },
                status=status.HTTP_400_BAD_REQUEST
            )

        application.hod = user
        application.hod_recommendation = recommendation or decision
        ...
        create_system_log(
            ...
            "HOD_DECISION",
            (
                f"HOD processed application {application.id}: "
                f"{decision}"
            ),
            request
        )
        ...
        return Response(
            {
                "message":
                "HOD decision recorded successfully.",
    ...
    # ========================================================
        ...
        if application.status != "DEAN_REVIEW":
            ...
            return Response(
                {
                    "error":
                    "This application is not currently available for Dean review.",
                    "current_status": application.status
                },
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if decision not in [
            ...
            return Response(
                {
                    "error":
                    "Invalid Dean decision. Use RECOMMENDED or REJECTED."
                },
                status=status.HTTP_400_BAD_REQUEST
            )

        application.dean = user
        application.dean_recommendation = recommendation or decision
        ...
        # ----------------------------------------------------
            ...
            # IMPORTANT:
            # Do NOT send directly to Committee.
            # The next stage is academic review.
        ...
        create_system_log(
            ...
            "DEAN_DECISION",
            (
                f"Dean processed application {application.id}: "
                f"{decision}"
            ),
            request
        )
        ...
        return Response(
            {
                "message":
                "Dean decision recorded successfully.",
    ...
    # ========================================================
        ...
        if application.status != "UNDER_REVIEW":
            return Response(
                {
                    "error": "This application is not currently available for academic review.",
                    "current_status": application.status,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # ----------------------------------------------------
        assignment_exists = ReviewerAssignment.objects.filter(
            application=application, reviewer=user
        ).exists()

        if not assignment_exists:
            return Response(
                {
                    "error": "You are not assigned as a reviewer for this application."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        # ====================================================
        material_reviews = AcademicMaterialReview.objects.filter(
            material__application=application
        )

        total_material_reviews = material_reviews.count()

        completed_material_reviews = material_reviews.exclude(
            status__in=["PENDING", "DRAFT"]
        ).count()

        reviews_complete = (
            total_material_reviews > 0
            and completed_material_reviews >= total_material_reviews
        )
        ...
        #
        # IMPORTANT:
        # Student evaluations are NOT automatically related
        # to a promotion application merely because they belong
        # to the same instructor.
        #
        # Therefore, do NOT count the instructor's existing
        # evaluations as completed for this application.
        # ====================================================
        total_student_evaluations = 0
        completed_student_evaluations = 0
        evaluations_complete = False

        # ----------------------------------------------------
        # DO NOT MOVE APPLICATION TO COMMITTEE YET
        # ----------------------------------------------------
        application.status = "UNDER_REVIEW"
        ...
        create_system_log(
            user,
            "ACADEMIC_REVIEW_PROGRESS",
            (
                f"Academic review processed for application {application.id}. "
                f"Material reviews: {completed_material_reviews}/{total_material_reviews}. "
                f"Student evaluations: {completed_student_evaluations}/{total_student_evaluations}. "
        ...
        return Response(
            {
                "message": (
                    "Academic review recorded. "
                    "The application remains under review. "
                    "Student evaluation has not yet been processed "
                    "for this application."
                ),
                "application_id": application.id,
                "status": application.status,
                "academic_reviews": {
                    "completed": completed_material_reviews,
                    "total": total_material_reviews,
                    "complete": reviews_complete,
                },
                "student_evaluations": {
                    "completed": completed_student_evaluations,
                    "total": total_student_evaluations,
                    "complete": evaluations_complete,
                    "status": "NOT_STARTED",
                },
    ...
    # ========================================================
        ...
        if application.status != "COMMITTEE_REVIEW":
            ...
            return Response(
                {
                    "error":
                    "This application is not currently available for Committee decision.",
                    "current_status":
                    application.status
                },
                status=status.HTTP_400_BAD_REQUEST
        ...
        if decision not in [
            ...
            return Response(
                {
                    "error":
                    "Invalid Committee decision. Use APPROVED or REJECTED."
                },
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if decision == "APPROVED":
            ...
            old_title = application.employee.job_title
        ...
        create_system_log(
            ...
            "COMMITTEE_DECISION",
            (
                f"Promotion Committee decision for application "
        ...
        return Response(
            {
                "message":
                "Promotion Committee decision recorded successfully.",
    ...
    # ========================================================
        ...
        create_system_log(
            user,
            "ADMIN_PROMOTION_DECISION",
            (
                f"Administrator processed application "
                f"{application.id}: {decision}"
            ),
            request
        )
    ...
    return Response(
        {
            "error":
            "You are not authorized to process this promotion application."
        },
        status=status.HTTP_403_FORBIDDEN
    )
...
def promotion_materials(request):
    ...
    reviews = AcademicMaterialReview.objects.all()

    if application_id:
        reviews = reviews.filter(
            material__application_id=application_id
    )
...
@api_view(["GET", "POST"])
@permission_classes([IsAuthenticated])
def manage_teaching_evaluations_list(request):
    ...
    if request.method == "GET":
        ...
        # CURRENT USER FILTER
        #
        # Only return evaluations belonging to applications
        # owned by the currently logged-in staff member.
        # --------------------------------------------------------
...
def _filter_queryset_for_user(model, queryset, request):
    user = request.user

    if model is PromotionApplication:
        if user.role == "STAFF":
            ...
            queryset = queryset.filter(dean=user)
        elif user.role == "REVIEWER":
            queryset = queryset.filter(assigned_reviewer=user)
        elif user.role == "COMMITTEE":
            queryset = queryset.filter(status="COMMITTEE_REVIEW")
        elif user.role == "ADMIN":
            queryset = queryset.all()
        else:
            queryset = queryset.none()

    elif model is StudentTeachingEvaluation:
        if user.role == "STUDENT":
            queryset = queryset.filter(student=user)
        elif user.role == "STAFF":
            queryset = queryset.filter(instructor=user)
        elif user.role in ["HOD", "DEAN", "ADMIN", "COMMITTEE", "REVIEWER"]:
            queryset = queryset.all()
        else:
            queryset = queryset.none()

    elif model is AcademicMaterialReview:
        if user.role == "REVIEWER":
            queryset = queryset.filter(reviewer=user)
        elif user.role == "STAFF":
            queryset = queryset.filter(material__application__employee=user)
        elif user.role in ["HOD", "DEAN", "ADMIN", "COMMITTEE"]:
            queryset = queryset.all()
        else:
            queryset = queryset.none()

    elif model is PromotionMaterial:
        if user.role == "STAFF":
            queryset = queryset.filter(application__employee=user)
        elif user.role in ["REVIEWER", "COMMITTEE", "ADMIN", "DEAN", "HOD"]:
            queryset = queryset.all()
        else:
            queryset = queryset.none()

    elif model is ReviewerAssignment:
        if user.role == "REVIEWER":
            queryset = queryset.filter(reviewer=user)
        elif user.role == "STAFF":
            queryset = queryset.filter(application__employee=user)
    ...
    elif model is PromotionAppeal:
        if user.role == "STAFF":
            queryset = queryset.filter(applicant=user)
        elif user.role in ["APPEAL_COMMITTEE", "ADMIN"]:
            queryset = queryset.all()
        else:
            queryset = queryset.none()

    elif model is PromotionHistory:
        if user.role == "STAFF":
            queryset = queryset.filter(employee=user)
        elif user.role in ["ADMIN", "COMMITTEE", "DEAN", "HOD"]:
            queryset = queryset.all()
        else:
            queryset = queryset.none()
    ...
    application_id = request.query_params.get("application")
    if application_id:
        if model is AcademicMaterialReview:
            queryset = queryset.filter(material__application_id=application_id)
        elif model is PromotionMaterial:
            queryset = queryset.filter(application_id=application_id)
        elif model is PromotionApplication:
            queryset = queryset.filter(id=application_id)
        elif model is ReviewerAssignment:
            queryset = queryset.filter(application_id=application_id)
        elif model is StudentTeachingEvaluation:
            try:
                application = PromotionApplication.objects.get(id=application_id)
            except PromotionApplication.DoesNotExist:
                queryset = queryset.none()
            else:
                queryset = queryset.filter(instructor=application.employee)
...
def generic_api(model, serializer_class):
    ...
    def list_create(request):
        ...
        if request.method == "GET":
            ...
            queryset = _filter_queryset_for_user(model, queryset, request)
